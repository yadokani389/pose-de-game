...
impl OpenVinoBackend {
    pub(crate) fn load(pose: &ModelData, seg: &ModelData, input_size: u32) -> Result<Self> {
        ...
        let pose = Self::build_session(&mut core, pose, input_size, 1)?;
        let seg = Self::build_session(&mut core, seg, input_size, 2)?;
    ...
    fn build_session(
        ...
        let model = core
            .read_model_from_buffer(model.bytes(), None)
            .with_context(|| format!("failed to read model ({})", model.label()))?;
