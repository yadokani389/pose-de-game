...
fn load_model_data(path: Option<&Path>, embedded: EmbeddedModel) -> Result<ModelData> {
    match path {
        Some(path) => {
            let label = path.display().to_string();
            let bytes =
                fs::read(path).with_context(|| format!("failed to read model at {label}"))?;
            Ok(ModelData {
        ...
        None => Ok(ModelData {
            bytes: Cow::Borrowed(embedded.bytes),
            label: Cow::Borrowed(embedded.name),
        }),
    }
}
...
pub struct InferenceOutput {
    ...
    pub frame_w: u32,
    pub frame_h: u32,
    pub _pose_output_shape: Vec<usize>,
    pub _seg_output_shape: Vec<usize>,
    pub _proto_shape: Vec<usize>,
}
...
impl PoseSegPipeline {
    pub fn new(
        backend: BackendKind,
        pose_model: Option<&Path>,
        seg_model: Option<&Path>,
        require_cuda: bool,
        enable_seg: bool,
    ) -> Result<Self> {
        let pose_data = load_model_data(pose_model, EMBEDDED_POSE_MODEL)?;
    ...
    pub fn infer_profiled(
        ...
        let (people, seg_shape, _proto_shape) = if let Some(seg_raw) = seg_raw_opt {
        ...
        let output = InferenceOutput {
            ...
            frame_w,
            frame_h,
            _pose_output_shape: pose_raw.dims.clone(),
            _seg_output_shape: seg_shape,
            _proto_shape,
        };
