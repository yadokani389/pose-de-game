...
fn main() {
    ...
    let pose_model =
        load_model(&args.model, args.require_cuda).expect("failed to load ONNX Runtime model");
...
fn load_model(path: &str, require_cuda: bool) -> Result<PoseModel> {
...
fn run_pose_inference(
    ...
    let input_tensor = Tensor::from_array((
        vec![
            1usize,
            3,
            pose_model.input_size as usize,
            pose_model.input_size as usize,
        ],
        input_data,
    ))?;

    let outputs = pose_model.session.run(ort::inputs![
        pose_model.input_name.as_str() => input_tensor
    ])?;

    let output = outputs
        .get(&pose_model.output_name)
        .context("missing model output")?;
    ...
    let Some((best_idx, layout, channels, detections, best_score)) =
        find_best_detection(data, &dims)
    else {
        return Ok((Vec::new(), 0.0, dims));
    };

    let keypoints = decode_keypoints(
        data, &dims, best_idx, layout, channels, detections, &letterbox,
    );
